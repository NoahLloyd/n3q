...
export async function fetchPolls(userId: string): Promise<Poll[]> {
  ...
  const pollsWithInfo = await Promise.all(
    (polls || []).map(async (poll) => {
      ...
      const creator = await getProfile(poll.creator_id);
...
export async function createPoll(
  ...
  return fetchPoll(poll.id, userId);
}
...
export async function castVote(
  ...
  const { shouldClose, winner } = await checkAutoClose(pollId, totalMembers);
  ...
  return fetchPoll(pollId, userId);
}
